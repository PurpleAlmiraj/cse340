...
Util.buildItemGrid = async function(data) {
  ...
  try {
    if (data != null) {
      ...
      grid += '<img id="vehicle-img" src="' + data.inv_image 
        + '" alt="Image of ' + data.inv_make + ' ' + data.inv_model 
        + ' on CSE Motors" />';
      grid += '<h2 id="vehicle-title">' + data.inv_make + ' ' + data.inv_model + ' Details</h2>';
      grid += '<h3 id="vehicle-price">Price: $' 
        + new Intl.NumberFormat('en-US').format(data.inv_price) + '</h3>';
      grid += '<div id="vehicle-description"><h3>Description: </h3><p>' 
